...
void *serverConversation(int sockfd)
{
    ...
    if (word == NULL){
        ...
        word = strtok(NULL, " ");
        char *targetdir = findRepoFilepath(word, 1);
        printf("targetdir is %s", targetdir);
        createZip(word, 1);
        
        long int checkoutfilesize = findFileSize("./Server/data/checkout.tar.gz");
        printf("\n Checkoutfilesize is %d", checkoutfilesize);
        write(sockfd, (char *) &checkoutfilesize, sizeof(int));
        
        char *outfileBuffer = readFileToBuffer("./Server/data/checkout.tar.gz");
        outfileBuffer[checkoutfilesize+1] = '\0';
        int i = 0;
        ...
        char *manifestfilepath = findManifestFilepath(word, 1);
...
//        FILE *fp = fopen("./Server/data/checkout.tar.gz", "wb");
        
//        fprintf(fp, "%s", incomingData);
        // Due to issues with above command I am using below line to simulate
        printf("\n D S 235\n");
        
        system("cp -rf ./Client/data/checkout.tar.gz ./Server/data/checkout.tar.gz");
        system("tar -xzvf ./Server/data/checkout.tar.gz -C ./Server/data");
        
        moveDataToServerManifest(word, 1);
        update(word, 1);
        resolve("./Server/data/servermanifest", findManifestFilepath(word, 1), findDataRepo(word, 1), findRepoFilepath(word, 1), 1);
...
//        fclose(fp);
        ...
        // THIS AREA OF CODE SHOULD NOT BE ACCESSED
        ...
        word = strtok(NULL, " ");
        int currentversion = getManifestVersion(findManifestFilepath(word, 1));
        write(sockfd, (char *) &currentversion, sizeof(char));
